...
contract BoredNFT is ERC721Enumerable, ReentrancyGuard, Ownable {
    string public _baseTokenURI;
    uint256 public _reserved = 150;
    uint256 public _maxBoredPerWallet = 2;
    ...
    constructor(
        ...
        // team gets the  first boredNFT for giveaway
        _safeMint(team, 0);
    }
    ...
    function summon(uint256 amount) external payable {
        ...
        require(
            amount + balanceOf(msg.sender) <= _maxBoredPerWallet && amount > 0,
            "!BoredAmount"
        );
    ...
    function setMaxBoredPerWallet(uint256 _newMax) external onlyOwner {
        _maxBoredPerWallet = _newMax;
    }
